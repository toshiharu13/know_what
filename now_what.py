...
def knowuser(ARM, way_to):
    ...
    os.system(f'wmic.exe /node:\"{ARM}\" computersystem get username >{way_to}\\temp\\{ARM}.txt')
    with open(f'{way_to}\\temp\\{ARM}.txt') as user_tmp:
        linecount = 0
        for tmp in user_tmp:
            tmp = tmp.strip()
            linecount += 1
...
def know_OS(ARM,way_to):
    ...
    os.system(f'WMIC /NODE:\"{ARM}\" OS get Caption >{way_to}\\temp\\{ARM}.txt')
    with open(f'{way_to}\\temp\\{ARM}.txt') as version_tmp:
        linecount = 0
...
def Start(ivent):
    ...
    global user, pasw
    user = f'gmc\\{enter_login.get()}'  # имя пользователя с правами админа
    pasw = enter_passw.get()  # пароль

    makdir_files() # проверяем темп папку, если нет, создаём
    ...
    with open(f'{way_to}\\list.txt') as list_of_arms:
        for ARM in list_of_arms:
            ...
            if if_ARM_online(ARM) == False:
                ...
                print(f'Name of ARM {ARM}')
                print(f'IP address of ARM: {IP_address}')
                ...
                if tmpprogramm != '':
                    print(f'now installed {tmpprogramm}')
                else:
                ...
                print(f'Now working: {user_DT} user')
                logwritting(user_DT)
...
linebreake = '********************'
# устанавливаем динамический путь до программы
...
lbl_log = tkinter.Label(main_window, text = f'файл лога: {way_to}\\log.txt', font = ("Arial", 8, 'bold'))
lbl_list = tkinter.Label(list_arm_window, text =f'список АРМ-ов: {way_to}\\list.txt', font = ("Arial", 8, 'bold'))
